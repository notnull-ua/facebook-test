...
abstract class AbstractService implements ServiceInterface
{
    ...
    {
        if (!is_array($config))
            throw new \ErrorException(__METHOD__."expects an array with keys: `client_id`, `client_secret`, `redirect_uri`");

        foreach (['client_id', 'client_secret', 'redirect_uri'] as $param) {
            if (!array_key_exists($param, $config)) {
                throw new \ErrorException(__METHOD__ . ' expects an array with key: `' . $param . '`');
            } else {
